...
```{r volc2, message=FALSE, warning=FALSE}
...
```{r go6, message=FALSE, warning=FALSE}
troch_vs_egg_GO <- run_GO_analysis(troch_vs_egg$up, oli_geneID2GO)
...
```{r tmp_heatmap}
neural_retina <- get_term_genes(troch_vs_egg$up, "GO:0021871", oli_geneID2GO, annotation)
zscore <- make_zscore_matrix(neural_retina, normalized_counts)
pheatmap(zscore, cluster_cols = FALSE, fontsize = 8, angle_col = 0, 
         treeheight_row = 30,
         color = viridis(n = 20, option = "inferno" ), 
...
```{r tmp_heatmap}
fate <- get_term_genes(troch_vs_gastr$down, "GO:0048863", oli_geneID2GO, annotation)
zscore <- make_zscore_matrix(fate, normalized_counts)
pheatmap(zscore, cluster_cols = FALSE, fontsize = 8, angle_col = 0, 
         treeheight_row = 30,
         color = viridis(n = 20, option = "inferno" ), 
...
```{r go6, message=FALSE, warning=FALSE}
adult_vs_egg_GO <- run_GO_analysis(adult_vs_egg$up, oli_geneID2GO)
...
```{r}
hox_genes <- rbind(subset(blast_vs_egg_up_interesting, grepl('HOX', Preferred_name)),
                   subset(blast_vs_egg_down_interesting, grepl('HOX', Preferred_name)),
                   subset(gastr_vs_blast_up_interesting, grepl('HOX', Preferred_name)),
                   subset(gastr_vs_blast_down_interesting, grepl('HOX', Preferred_name)),
                   subset(troch_vs_gastr_up_interesting, grepl('HOX', Preferred_name)),
                   subset(troch_vs_gastr_down_interesting, grepl('HOX', Preferred_name)),
                   subset(adult_vs_troch_up_interesting, grepl('HOX', Preferred_name)),
                   subset(adult_vs_troch_down_interesting, grepl('HOX', Preferred_name)))

hox_genes <- unique(hox_genes[c('gene', 'Preferred_name')])
hox_genes$query <- hox_genes$gene

zscore <- make_zscore_matrix(hox_genes, normalized_counts)
pheatmap(zscore, cluster_cols = FALSE, fontsize = 8, angle_col = 0, 
         treeheight_row = 30,
...
gastr_vs_blast_all <- gastr_vs_blast$all
blast_vs_egg_all <- blast_vs_egg$all
troch_vs_gastr_all <- troch_vs_gastr$all
adult_vs_troch_all <- adult_vs_troch$all
```